...
#else
void tBindBase::Init(SX1268* radio_ref)
{
    // Set up the radio reference
    radioBind = radio_ref;
    // Continue with the rest of the initialization
...
void tBindBase::handle_receive(uint8_t antenna, uint8_t rx_status)
{
    ...
    strncpy(BIND_PHRASE, txBindFrame.BindPhrase_6, 6);
...
void tBindBase::do_transmit(uint8_t antenna)
{
    ...
    rxBindFrame.firmware_version = 01;
    strncpy(rxBindFrame.device_name_20, "T1000", 20);

    rxBindFrame.crc = fmav_crc_calculate((uint8_t*)&rxBindFrame, FRAME_TX_RX_LEN - 2);
    radioBind->transmit((uint8_t*)&rxBindFrame, sizeof(rxBindFrame));
}


uint8_t tBindBase::do_receive(uint8_t antenna, bool do_clock_reset)
{
    radioBind->receive((uint8_t*)&txBindFrame, sizeof(txBindFrame));
    ...
    if (ok && do_clock_reset) esp_restart();
